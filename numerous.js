...
Numerous.prototype.getChannelMetricsForUser = function(userId, token, callback) {
	var self = this;
	self.makeChannelRequest("GET", 'https://api.numerousapp.com/v2/channels/' + self.channelId + '/users/' + userId + '/metrics', null, token, callback);
}
...
Numerous.prototype.getChannelMetricWithSourceClassAndKey = function(sourceClass, sourceKey, callback) {
	var self = this;
	self.makeRequest("GET", 'https://api.numerousapp.com/v2/channels/' + self.channelId + '/metrics/' + sourceClass + '/' + sourceKey, undefined, callback);
}
